...
def get_count_pseudobulk(adata, layer=None, split_by=None, use_raw=False):
    ...
    if split_by is None:
        if use_raw:
            df_res = pd.DataFrame({"counts" : np.matrix(adata.raw.X.sum(axis=0)).toarray()[0]})
        elif layer is None:
            df_res = pd.DataFrame({"counts" : np.matrix(adata.X.sum(axis=0)).toarray()[0]})
        else:
            df_res = pd.DataFrame({"counts" : np.matrix(adata.layers[layer].sum(axis=0)).toarray()[0]})
    else:
        df_res = pd.DataFrame()
        for splitter in set(adata.obs[split_by]):
            if use_raw:
                df_res[splitter] = np.matrix(adata[adata.obs[split_by] == splitter].raw.X.sum(axis=0)).toarray()[0]
            elif layer is None:
                df_res[splitter] = np.matrix(adata[adata.obs[split_by] == splitter].X.sum(axis=0)).toarray()[0]
            else:
                df_res[splitter] = np.matrix(adata[adata.obs[split_by] == splitter].layers[layer].sum(axis=0)).toarray()[0]
    if use_raw:
...
def draw_panel(adata, genes, title="", heigh=4, dpi=150):
    ...
    if len(genes) == 1:
        ...
        scv.pl.umap(adata, color=genes[0], ax=ax, show=False, cmap=beautiful_cmap())
        plt.suptitle(title)
    else:
        fig, axes = plt.subplots(ncols=len(genes), figsize=(heigh * len(genes) * 0.8, heigh), dpi=dpi)
        counter = 0
        for gene in genes:
            scv.pl.umap(adata, color=gene, ax=axes[counter], show=False, cmap=beautiful_cmap())
            counter += 1
        fig.suptitle(title)
        fig.tight_layout()
...
def draw_multiple_cloneIDs(adata, FP):
    ...
    sns.boxplot(x=f"cloneID_multiplet_{FP}",
                y="doublet_score",
                data=adata.obs,
                ax=axes[0],
                order=order,
...
def predict_doublets(mdata, FP, min_cloneID_umi=2):
    ...
    if isinstance(FP, str):
        ...
        df_FP1 = pd.DataFrame(
            mdata[FP1].X.toarray() >= min_cloneID_umi,
            index=mdata[FP1].obs.index,
            columns=mdata[FP1].var.index
        )
        df_FP2 = pd.DataFrame(
            mdata[FP2].X.toarray() >= min_cloneID_umi,
...
def draw_expression_distribution(adata, FP1, FP2):
    ...
    sns.kdeplot(adata[adata.obs["cloneID_multiplet"] == FP1, FP1].X.toarray().T[0], color="green", fill=True,
                ax=axes[0], label=FP1)
    sns.kdeplot(adata[adata.obs["cloneID_multiplet"] == FP1, FP2].X.toarray().T[0], color="red", fill=True,
                ax=axes[0], label=FP2)
    ...
    sns.kdeplot(adata[adata.obs["cloneID_multiplet"] == FP2, FP1].X.toarray().T[0], color="green", fill=True,
                ax=axes[1], label=FP1)
    sns.kdeplot(adata[adata.obs["cloneID_multiplet"] == FP2, FP2].X.toarray().T[0], color="red", fill=True,
                ax=axes[1], label=FP2)
    ...
    sns.kdeplot(adata[adata.obs["cloneID_multiplet"] == "Unknown", FP1].X.toarray().T[0], color="green", fill=True,
                ax=axes[3], label=FP1)
    sns.kdeplot(adata[adata.obs["cloneID_multiplet"] == "Unknown", FP2].X.toarray().T[0], color="red", fill=True,
                ax=axes[3], label=FP2)
...
def resolve_confusion(adatas_FP):
    ...
    mean_logFC = np.true_divide(
        difference.sum(axis=0).toarray()[0],
        (difference != 0).sum(axis=0).toarray()[0]
    )
    cell_difference = (
        ((adatas_FP[FP1][:, ambiguous_bc].X - adatas_FP[FP2][:, ambiguous_bc].X) > 0).sum(axis=0).toarray()[0] -
        ((adatas_FP[FP2][:, ambiguous_bc].X - adatas_FP[FP1][:, ambiguous_bc].X) > 0).sum(axis=0).toarray()[0]
    )
    final_assignment = []
...
def draw_gRNAs_in_clones(CRISPR_table, FP_clones, num_clones=5, title="gRNAs in top clones"):
    gRNA_consistency = []
    target_clones = FP_clones.value_counts()[1:(num_clones + 1)].index

    fig, axes = plt.subplots(ncols=len(target_clones), figsize=(12, 5))

    i = 0
    for clone in target_clones:
        clone_cells = FP_clones[FP_clones == clone].index
        CRISPR_table.loc[clone_cells]["feature_call"].astype(str).value_counts().plot.bar(
            ax=axes[i],
            edgecolor="black",
            width=0.75,
        )
        axes[i].grid(alpha=0.3)
        if i == 0:
            vmax = len(clone_cells)
        axes[i].set_ylim(0, vmax + 1)
        axes[i].set_title(clone)
        plt.setp(axes[i].get_xticklabels(), ha="right", rotation=30)
        i += 1
...
def draw_gRNA_pies(adatas_FP, CRISPR_table, min_cloneID_umi=2, title="Basic gRNA statistics"):
    gRNAs = []
    for feature in CRISPR_table.feature_call:
        if type(feature) != float:
            if "|" in feature:
                gRNAs += feature.split("|")
            else: 
                gRNAs.append(feature)
        else:
            gRNAs.append("None")
                
    gRNAs = pd.Series(gRNAs).value_counts()
    gRNA_counts = CRISPR_table.num_features.value_counts()
    has_gRNA = (CRISPR_table.num_features != 0).values

    fig, axes = plt.subplots(
        ncols=(len(adatas_FP) + 2),
        figsize=(12, 12 // (len(adatas_FP) + 2)),
    )

    i = 0
    for FP in adatas_FP:
        has_clonalID = (adatas_FP[FP].X >= 2).sum(axis=1).T.toarray()[0] > 0

        both = (has_clonalID & has_gRNA).sum()
        gRNA = (~has_clonalID & has_gRNA).sum()
        clonalID = (has_clonalID & ~has_gRNA).sum()
        none = (~has_clonalID & ~has_gRNA).sum()

        axes[i].pie(
            x=[both, gRNA, clonalID, none],
            labels=["CloneID + gRNA", "gRNA", "CloneID", "None"],
            autopct="%.0f%%",
            wedgeprops={"edgecolor": "black"},
            textprops={"size": 12},
        )
        axes[i].set_title(f"{FP} cloneID and gRNA calling")
        i += 1
    ...
    axes[-2].set_title("Different gRNAs calling")

    axes[-1].pie(
        x=gRNA_counts.values,
        labels=gRNA_counts.index,
        autopct="%.0f%%",
        wedgeprops={"edgecolor": "black"},
        textprops={"size": 12},
    )
    axes[-1].set_title("Number of gRNAs per cell")
...
def gex_qc_report(adata_raw, adatas_FP, mt_name="mt-", mt_max=25, savefig="output.pdf",
    ...
    with sns.axes_style("ticks"):
        ...
        del adata # <================================================================== I added this line since it crashes which I think is due to memory constrains. 
        ...
        if not (CRISPR_table is None):
            adata_clear.obsm["CRISPR"] = CRISPR_table
            for FP in adatas_FP:
                FP_clones = fast_clones(adatas_FP[FP], min_cloneID_umi=min_cloneID_umi)
                figs.append(draw_gRNAs_in_clones(
                    adata_clear.obsm["CRISPR"],
                    FP_clones,
                    title=f"gRNAs in top {FP} clones"
                ))
            figs.append(draw_gRNA_pies(
                adatas_FP,
                adata_clear.obsm["CRISPR"],
        ...
        for FP in list(adatas_FP.keys()):
            df = pd.DataFrame(
                adatas_FP[FP].X.toarray() >= min_cloneID_umi,
        ...
        if len(adatas_FP) == 2:
            ...
            df_FP1 = pd.DataFrame(
                adatas_FP[FP1].X.toarray() >= min_cloneID_umi,
                index=adatas_FP[FP1].obs.index,
                columns=adatas_FP[FP1].var.index
            )
            df_FP2 = pd.DataFrame(
                adatas_FP[FP2].X.toarray() >= min_cloneID_umi,
        ...
        for FP in list(adatas_FP.keys()):
            df = pd.DataFrame(
                adatas_FP[FP][adata_clear.obs_names].X.toarray() >= min_cloneID_umi,
...
def prepare_embedding(adata, name="", DATA_PATH="/home/sergey/data/clonal_project"):
    ...
    sp.tl.map_embedding(adata_cycling, adata, key="sample_id")
    sp.tl.per_cell_confidence(adata_cycling, adata)
    adata_full = adata.raw.to_adata().concatenate(adata_cycling, index_unique=None, batch_key=None, uns_merge="first")
    for var in [
        "highly_variable", "highly_variable_rank", "means", "variances",
        "variances_norm", "highly_variable_nbatches"
    ]:
        del adata_full.var[f"{var}-0"]
        
    adata_full.raw = adata_full
    adata_full = adata_full[:, adata.var_names]
    adata_full.var["mean"] = adata.var["mean"]
    adata_full.var["std"] = adata.var["std"]
    adata_full.var["highly_variable"] = True
    adata_full.varm["PCs"] = adata.varm["PCs"]
    
    sc.pp.neighbors(adata_full, n_neighbors=20, use_rep="X_pca_harmony")
    sc.tl.umap(adata_full)
    
    for res in [0.5, 1, 1.5, 3]:
        sc.tl.leiden(adata_full, resolution=res, key_added=f"leiden_{res}")
        
    adata_full.raw.to_adata().write_h5ad(f"{DATA_PATH}/cellxgene/{name}.h5ad")
    adata_full.write_h5ad(f"{DATA_PATH}/anndatas/{name}_reference.h5ad")
    adata.write_h5ad(f"{DATA_PATH}/anndatas/{name}_noncycling_reference.h5ad")